...
- 2026-06-20 — audited through `1f9f001`. Same window also saw the web dashboard removed from the fork entirely (`refactor!: remove web dashboard`), which mooted the dashboard-related upstream work. Backported across two commits: `feat(security)!` carries the still-relevant pieces of `f7ea10f` (#185) — fail-closed allowlists (Telegram + Discord deny on empty `allowedUserIds`; new `discord.allowedGuilds`; daemon refuses to start a bot with an empty allowlist; Telegram leaves groups added by unauthorized users), the `prompt-safety.ts` `wrapUntrusted()` wrapper applied to message text/skill args/voice transcripts/filenames/guild+group names, and the runner untrusted-content system-prompt line; the web-auth and `/api/technical-info` redaction pieces of `f7ea10f` were dropped (no web). A `feat:` commit carries whisper-model config (`7a76717`+`276e7e3` → `telegram.whisperModel`), `/mode` permission modes (`34fa655` runner half + shared `/mode` built-in, wired for Telegram text and a Discord slash command), Telegram accept-any-file (`1f9f001` core: dropped the MIME whitelist), and Discord forwarded-message coalescing reimplemented for discord.js (`f54be39`+`3705a36`+`c9fc65e`). Skipped — fork has no Slack: `88a781b 9758f69 6e71aab fc08145 937ecb0 0c3d624 c99d3d7 ff4f7a9`. Skipped — upstream raw-HTTP/gateway-specific, handled by discord.js in the fork: `bd897f0 8e8f3a6 389276b dda8441 3f7f87b`. Skipped — fork has no job-retry/backoff system to feed: `ed9a96c afa66ef 6c5e201`. Skipped — `7851307` (telegram `/model*` commands; fork's `model-router.ts` already does automatic model selection). Partial — `1f9f001`'s inline gallery-media handling (video/sticker/animation/video_note) not ported (separate download wiring). Deferred (web removed, so N/A): dashboard session-usage tab (`4157171`), web-ui file attachments (`610aa20`), Discord streaming preview (`d8b9558`). Not relevant: version-bump chores, `eeefc3e` (bootstrap docs).

- 2026-05-07 — audited through `ae5ecd4`. Backported across two commits on `main`: `3017840` (`fix:` bundle) carries `1fbbdb3` (statusline alive() PID validation), `ba894fd` (auto-recover from stale --resume session IDs in runner exec + stream paths), `d9c2fcc` (job frontmatter snapshot/restore + thinking-block signature recovery — combined with stale recovery as a single backup-and-retry path), `c2380fd`+`7d1b31c` (pause heartbeats/jobs/chat when rate-limited; runner now exposes `isRateLimited()`/`getRateLimitResetAt()`), and `6484b92` (job lastResult/lastRanAt persisted in state.json); a follow-up `feat(discord)` commit carries `a3cf80b` (reply/forward context, generalised in chat-handler so Telegram can be wired later). Already in fork: `5cd2522` (rate-limit "out of extra usage" pattern), `73114b7` (rejoined-thread count), `d2886f7` (job notify frontmatter). Skipped — depends on subsystems not in fork: per-agent fallback sessions (`79107c3 e6aca97 73cf44b 64fc22d 56e886f aecaa4e b411eda`), slack (`69874b1 172dd23 c335730 923a293 1144dd3`), plugin/wizard (`a421b4b c263ce0`), Discord raw HTTP 429 retries (`b24bb86 911a878` — fork uses discord.js), agent-text XSS (`9df8278` — fork's UI lacks the chat-msg-agent path), telegram polling generation token (`895c9df` — fork has no stopPolling toggle). Skipped — Linux/Pi target: `87dcd7f` (Windows claude.cmd bypass). Deferred features (no urgency): operation journal, secrets vault, session auto-rotation, /api/inject, pending-resume, active-runs counter, web session browser, sub-agent lifecycle events, per-context timeouts, STT delegateTool, telegram /fork /kill /buttons /sendVoice, discord image attachments, telegram per-topic isolation + dmIsolation, listenGuilds. Not relevant: CONTRIBUTING.md (fork is GPL), version-bump chores, docs-only commits.
